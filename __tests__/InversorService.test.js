...
describe('Teste Unitário: InversorService', () => {
    // CRÍTICO: Instancia a classe InversorService para usar seus métodos de instância
    const service = new InversorService(); 
    
    // Cenário 1: Entrada válida (Cobre a lógica principal)
    test('1. Deve inverter uma palavra simples corretamente', () => {
        // Agora chama o método na instância 'service'
        expect(service.inverterString('teste')).toBe('etset'); 
    });
    ...
    test('2. Deve inverter frase com espaços e manter a ordem dos caracteres', () => {
        expect(service.inverterString('ola mundo')).toBe('odnum alo');
    });
